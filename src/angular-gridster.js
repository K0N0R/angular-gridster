			...
			function ($document, $window, GridsterTouch, $timeout) {
				function GridsterDraggable($el, scope, gridster, item, itemOptions) {
					...
					function inScrollRange(event) {
						...
						if (hasY) {
							rY = { top: 0, bottom: 0, right: 0, left: 0 };
							rY.top = e.offset().top;
						...
						if (hasX) {
							rX = { top: 0, bottom: 0, right: 0, left: 0 };
			...
			function ($parse, GridsterDraggable, GridsterResizable, gridsterDebounce, $rootScope) {
				return {
					...
					link: function (scope, $el, attrs, controllers) {
						...
						var mouseupTimeoutHandler;
						$(document).on('mousedown.gridsterhandler', function (event) {
							if ($(event.target).hasClass('gridster-item') || $(event.target).parents('.gridster-item')) {
								sendNotification = true;
								clearTimeout(mouseupTimeoutHandler);
							}
						});

						$(document).on('mouseup.gridsterhandler', function (event) {
							clearTimeout(mouseupTimeoutHandler);
							mouseupTimeoutHandler = setTimeout(function () {
								sendNotification = false;
							}, 1000);
						});

						scope.$on('$destroy', function () {
							$(document).off('mousedown.gridsterhandler');
							$(document).off('mouseup.gridsterhandler');
						});
						...
						var debouncedTransitionEndPublisher = gridsterDebounce(function () {
							scope.$apply(function () {
								$rootScope.$broadcast('gridster-item-transition-end', item, gridster);
							});
						}, 50);
